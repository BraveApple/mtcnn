...
data_root = '/home/wang/disk_4T/wang_data/original_dataset/IMDB-WIKI';
anno_root = fullfile(data_root, 'Anno');
img_root = fullfile(data_root, 'Img/Img_Raw');
...
crop_root = fullfile(output_root, 'crop_image');
...
list_txt = fullfile(anno_root, 'img_raw.txt');
...
for i = 1:length(img_list)
  ...
  if save_crop
    ...
    pt2 = boundingboxes(max_id, 3:4);
    center_pt = pt2 - pt1;
    pt = center_pt - crop_size / 2;
    roi = [pt(1), pt(2), crop_size(1), crop_size(2)]; 
    img_crop = cv.Rect.crop(img_crop, roi);
    crop_img_path = fullfile(crop_root, img_subpath);
    [root, name, ext] = fileparts(crop_img_path);
    if ~exist(root, 'dir')
      mkdir(root);
    end
    cv.imwrite(crop_img_path, img_crop);
